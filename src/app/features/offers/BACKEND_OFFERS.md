...
- **Offers:**
  ...
  - `PATCH /api/offers/{id}/status` — transition (`UpdateOfferStatusDto`: status = SENT | VIEWED | ACCEPTED | REJECTED | EXPIRED).
- **Bookings:** `POST /api/bookings` with `CreateBookingDto` (offerId). Filter/update status: PENDING, CONFIRMED, PAID, CANCELLED.
- **Requests:** GET/POST /api/requests, GET/PATCH/DELETE /api/requests/{id}, PATCH /api/requests/{id}/status. Filter/update status: DRAFT, IN_PROGRESS, WAITING_CLIENT, CLOSED. `RequestResponseDto`, `UpdateRequestDto`, `UpdateRequestStatusDto` in OpenAPI.
